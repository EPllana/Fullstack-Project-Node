...
export const sendVerificationEmail = async (email, name, verificationToken) => {
  console.log(`Sending verification email to: ${email}`);  // Sigurohu që email-i është i saktë
  console.log(`Verification Token: ${verificationToken}`);  // Kontrollo vlerën e token-it

  if (!email) {
    console.error("❌ No email provided, can't send verification email!");
    return; // If no email is provided, don't send email
  }
  ...
  try {
    const info = await transporter.sendMail({
      ...
      html: `<p>Hello ${name}, please verify your email by clicking <a href="${verificationToken}">here</a>.</p>`,
    });
