...
export const getAllBooking = async (req,res)=>{
  try{
    ...
    const booking = await Bookings.find(filter).populate('tour','title price description').populate('user','name email').skip(skip).limit(limit).sort({bookingDate:-1});
    if(booking.length === 0){
      return res.status(404).json({ message: "No bookings found" });
    }
    const totalBooking = await booking.
