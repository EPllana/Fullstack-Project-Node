...
export const getAllBookings = async (req ,res)=>{
  ...
  if(search){
    filter.$or[
      {'tour.title':{$regex:search}}
    ]
  }

  const booking = await Bookinf.find(filter).populate('user','name email').populate('tour', 'title price descripition').skip(skip).limit(limit).sort({bookingDate:-1});
  if(booking.length ===0){
    return res.status(404)
  }
  const totalBookings = await Booking.countDocuments(filter);
  res.status(200).json({
    bookings,totalBookings,currentPage:page
  })
}
