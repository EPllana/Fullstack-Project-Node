...
export const getAllBookings = async (req, res)=>{
  try{
    ...
    const booking = await Booking.find(filter).populate('tour','title price description').skip(skip).limit(limit).sort({bookingDate:-1});

    if(bookings.length === 0){
      return res.status(404).json({message:"No booking founds"})
    }
    cont totalBookings = await Booking.countDocuments(filter)
