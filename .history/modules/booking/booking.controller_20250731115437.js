...
export const updateBookings = async(req,res)=>{
  ...
  if(!booking){
    ...
    if (guests) {const tour = await Tour.findById(booking.tour);

      if (!tour) {
        return res.status(404).json({ message: "Tour not found" });
      }
      booking.guests = guests;
      booking.totalPrice = tour.price * guests;  // Përditësojmë çmimin total
    }
    if (status) {
      booking.status = status;  // Përditësojmë statusin
    }
