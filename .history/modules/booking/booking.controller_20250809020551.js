...
export const createBooking = async (req, res) => {
  try {
    ...
    // Kontrollo që startDate dhe endDate janë të dhëna dhe janë strings të vlefshme
    if (!startDate || !endDate) {
      return res.status(400).json({ message: "Start date and end date are required" });
    }

    // Sigurohuni që startDate dhe endDate janë strings dhe mund të përdoren me .split()
