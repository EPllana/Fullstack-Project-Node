...
export const getAllBookings = async (req, res) => {
  try {
    const userId = req.user._id; // Sigurohuni që përdoruesi është autentifikuar
    const page = parseInt(req.query.page) || 1; // Përdor faqen 1 nëse nuk është dhënë
    const limit = parseInt(req.query.limit) || 10; // Përdor limitin 10 nëse nuk është dhënë

    const skip = (page - 1) * limit;

    let filter = { user: userId, isActive: true }; // Filter për të marrë vetëm rezervimet aktive
