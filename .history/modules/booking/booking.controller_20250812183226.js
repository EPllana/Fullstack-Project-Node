...
export const  getbookingStats = async (req,res)=>{
  try{
   ...
   const canceledBookings = await Booking.countDocuments({isActive:true, status:"cancel"});
...
//averageprice
 const avgPriceResults = await Booking.aggregate([{$match:{isActive:true}},
{$group:{_id:null,averagePrice:{$avg:"$totalPrice"}}}
]);

const avgPriceResults = avgPriceResults.length > 0 ? avgPriceResults[0].averagePrice: 0;

const startOfMonth = new Date(new Date().getFullYear(), new Date().getMonth(),1)// dita e par e mujit esht 1 

//me i filtru veq te mujit gusht psh nga timestamps kur jan kriju 
const thisMonthBookings = await Booking.countDocuments({
  isAcive:true,
  createdAt:{$gte:startOfMonth}//qka jan me t mdhaaja ose baraz se 1 gusht e ma e madhe po duhet me gjeneru fillimin e mujit me gjet ne mnyr dinamike  qaj sort of month mu gjeneru automatikisht first of month
})  
res.status(200).json({message:"Booking Statiscitcs",status:{totalBookings, bookingByStatus:{
  paidBookings,pendingBookings,canceledBookings}
},
totalRevenue,totalGuests,averagePriceResults,thisMonthBookings})
