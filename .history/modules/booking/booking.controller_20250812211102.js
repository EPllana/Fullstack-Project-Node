...
export const  getbookingStats = async (req,res)=>{
  try{
   ...
   const canceledBookings = await Booking.countDocuments({isActive:true, status:"canceled"});
...
const avgPrice = avgPriceResults.length > 0 ? avgPriceResults[0].avgPrice : 0;

const startOfMonth = new Date(new Date().getFullYear(), new Date().getMonth(), 1, 0, 0, 0, 0);  // Set to midnight (local time)
// dita e par e mujit esht 1 
...
totalRevenue,totalGuests,avgPrice,thisMonthBookings})
