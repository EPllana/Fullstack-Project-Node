...
import { sendVerificationEmail } from '../../config/email.js'; // Sigurohu që të kesh importuar email utility
...
export const createUser = async (req, res) => {
  try {
    ...
    // Krijojmë një token për verifikimin e email-it me ID-në e përdoruesit dhe afatin e skadimit 1 orë
    ...
    // Dërgojmë emailin e verifikimit tek përdoruesi me tokenin
    await sendVerificationEmail(user.email, user.firstName, verificationToken);

    // Kthejmë një përgjigje të suksesshme
    res.status(201).json({
      message: "Useri u krijua me sukses. Kontrollo email-in për të verifikuar llogarinë.",
    });
  } catch (error) {
    // Trajtojmë çdo gabim që mund të ndodhi
...
export const verifyEmail = async ({ email, name, verificationToken }) => {
  ...
  try {
    const info = await transporter.sendMail({
      ...
      to: email,  // Kontrollo që ky email është valid dhe i vërtetë
