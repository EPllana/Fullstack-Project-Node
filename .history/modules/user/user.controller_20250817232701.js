...
export const verifyEmail = async (req, res) => {
  try {
    // Log the incoming headers and query parameters for debugging
    console.log("Request Headers:", req.headers);  // Logs the headers
    console.log("Request Query:", req.query);      // Logs the query parameters

    // Extract token based on where it should come from
    const token = req.headers.authorization
      ? req.headers.authorization.split(" ")[1]  // If token is in Authorization header
      : req.query.token;                        // If token is in query parameters
