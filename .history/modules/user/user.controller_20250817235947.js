...
export const createUser = async (req, res) => {
  try {
    ...
    // Kthe përgjigjen për krijimin e përdoruesit
...
export const verifyEmail = async (req, res) => {
  try {
    ...
    // Kontrollo nëse tokeni është duke u kaluar si duhet
    console.log("Token i marrë nga URL:", token);

    if (!token) {
      return res.status(400).json({ message: "Token is required" });
    }

    // Gjej përdoruesin përmes tokenit
    const user = await User.findOne({ verificationToken: token });

    if (!user) {
      console.log("Përdoruesi nuk është gjetur për tokenin:", token);
      return res.status(400).json({ message: "Invalid or expired token" });
    }

    // Përditëso përdoruesin dhe fshij tokenin
