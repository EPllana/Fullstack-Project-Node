...
export const createUser = async (req, res) => {
  try {
    ...
    const verificationToken = jwt.sign(
      { userId: user._id },    // Përdorim `userId` nga përdoruesi që është ruajtur
      process.env.SECRET_KEY,   // Çelësi sekret për enkriptimin
      { expiresIn: "1h" }      // Token-i ka afat 1 orë
    );
    
    await sendVerificationEmail(user.email, user.firstName, verificationToken);  // Dërgojmë token-in në email
