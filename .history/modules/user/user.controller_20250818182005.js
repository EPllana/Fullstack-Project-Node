...
export const createUser = async (req, res) => {
  try {
    ...
    const existingUser = await User.findOne({ email }).sort({createdAt});    // Kontrollojmë në databazë nëse ekziston ndonjë user me email të njëjtë
    if (existingUser) {
...
export const verifyEmail = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Email-i u verifikua me sukses!" });
  } catch (error) {
