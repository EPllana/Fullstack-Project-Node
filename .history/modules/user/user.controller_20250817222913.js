...
export const verifyEmail = async (req, res) => {
  const { token } = req.query;  // Përdorimi i req.query për token-in, siç është përdorur më parë

  try {
    // Verifikojmë tokenin
    const decoded = jwt.verify(token, process.env.SECRET_KEY);

    // Gjejmë përdoruesin që ka dërguar këtë token
    ...
    // Aktivizojmë përdoruesin dhe vendosim isVerified në true
    ...
    await sendVerificationEmail({
      email: user.email,
      name: user.firstName,
      verificationToken: user.verificationToken,  // Sigurohu që përdoruesi ka verificationToken të vlefshëm
    });
