...
export const createUser = async (req, res) => {
  try {
    ...
    // Ruajmë përdoruesin në databazë
    await user.save();

    // Krijojmë një token verifikimi
    ...
    // Dërgojmë emailin e verifikimit
    await sendVerificationEmail(user.email, user.firstName, verificationToken);  // Sigurohu që po dërgon email-in dhe emrin e saktë
...
export const verifyEmail = async (req, res) => {
  try {
    ...
    console.log("Request Query:", req.query);  // Ensure we are seeing the token
    ...
    // Verify the token (optional: check if expired)
    try {
      jwt.verify(token, process.env.SECRET_KEY);  // If expired, this will throw an error
    } catch (error) {
    ...
    // Send success response (HTML message for the user)
    res.status(200).send(`
